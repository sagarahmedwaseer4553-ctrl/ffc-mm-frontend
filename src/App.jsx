  ...
  return (
    <div className="app">
      ...
      {currentPage === 'admin-dashboard' && adminAuthenticated && (
        <AdminDashboard setCurrentPage={setCurrentPage} setAdminAuthenticated={setAdminAuthenticated} />
      )}
    </div>
  );
}
      ...
      <main className="home-content">
        ...
        <div className="options-grid">
          <div className="option-card canteen-card">
            ...
            <button onClick={() => setCurrentPage('complaint')}>
              Report Issue →
            </button>
          </div>
...
function ComplaintForm({ setCurrentPage }) {
  ...
  const handleFileUpload = async (file) => {
    // Simple base64 encoding for file upload
            ...
            <div className="form-row">
              <div className="form-section">
                ...
                <input
                  ...
                  type="text"
                  name="designation"
                  placeholder="e.g. Engineer, Operator"
...
function AdminLogin({ onAuthenticate, setCurrentPage }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/admin/verify-pin`, { pin });
...
function AdminDashboard({ setCurrentPage, setAdminAuthenticated }) {
  ...
  const handleUpdateComplaint = async (id, updates) => {
    try {
      await axios.put(`${API_URL}/complaints/${id}`, updates, {
        headers: { adminPin }
      });
      fetchData();
  ...
  const handleAddEmail = async () => {
    if (newEmail && !emailConfig.recipients.includes(newEmail)) {
      const updated = {
        ...emailConfig,
        recipients: [...emailConfig.recipients, newEmail]
      };
      try {
        await axios.put(`${API_URL}/admin/email-config`, updated, {
          headers: { adminPin }
        });
  ...
  const getFilteredComplaints = () => {
    return complaints.filter(complaint => {
      ...
      const canteenMatch = canteenFilter === 'All' || complaint.canteen === canteenFilter;
  ...
  const generateReport = (complaint) => {
    const reportWindow = window.open('', '', 'height=500,width=800');
      ...
      <html>
        <head>
          ...
          <style>
            ...
            th { background-color: #2d7e5e; color: white; }
          </style>
        </head>
        <body>
